...
class SelfAttention(nn.Module):
    ...
    def __init__(self, embed_size, heads):
        ...
        self.O = nn.Linear(self.embed_size, self.embed_size, bias=False)
    ...
    def forward(self, values, keys, query, mask):
        ...
        xq, xkT = xq.permute(0, 2, 1, 3), xk.permute(0, 2, 3, 1)
        attention = xq @ xkT / self.head_dim**.5

        if mask is not None:
            mask[mask.eq(1)] = -torch.inf
            attention = attention + mask

        attention = F.softmax(attention, dim=-1)
        output = attention @ xv.permute(0, 2, 1, 3)

        output = output.permute(0, 2, 1, 3).reshape(N, value_len, -1)
...
def sinusoidal_positional_encoding(max_len: int, d_model: int):
    ...
    return torch.tensor(pe.astype(np.float32))


@torch.no_grad()
def construct_mask(x: torch.Tensor, padding_idx=0):
    mask = x.eq(padding_idx)  # [batch_size, seq_len]
    return torch.unsqueeze(mask, 1).unsqueeze(1)  # [batch_size, 1, 1, seq_len]
...
if __name__ == "__main__":
    ...
    ratio_pos = (train_y.float().mean() + 1) / 2
    ...
    for epoch in bar:
        ...
        for batch_X, batch_y in train_loader:
            ...
            loss = (weight.to(device) *
                    torch.log(1 + torch.exp(-label * torch.squeeze(output)))
                    ).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch_X, batch_y in train_loader:
                input_, label = batch_X.to(device), batch_y.to(device)
                pred = torch.sign(
                    torch.squeeze(net(input_, construct_mask(input_))))
