...
np.random.seed(42)
use_cuda = True
device = f'cuda:{torch.cuda.device_count() - 1}' if torch.cuda.is_available(
) else 'cpu'
...
train_loader = data_loader(torch.tensor(train_X), torch.tensor(train_y),
                           BATCH_SIZE, True)

train_accs, test_accs = [], []
test_X_cuda = torch.tensor(test_X, device=device)
test_y_cuda = torch.tensor(test_y, device=device)
...
for epoch in bar:
    ...
    with torch.no_grad():
        ...
        test_acc = np.array([
            pred1.eq(test_y_cuda).float().mean().item(),
            pred2.eq(test_y_cuda).float().mean().item(),
            pred3.eq(test_y_cuda).float().mean().item(),
        ])
